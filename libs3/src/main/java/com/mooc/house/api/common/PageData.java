...
public class PageData<T> {
    ...
    public static <T> PageData<T> buildPage(List<T>list, Long count, Integer pageSize, Integer pageNum){
        Pagination pagination = new Pagination(pageSize, pageNum, count, list.size());
        return new PageData<>(pagination, list);
    }

    public static <T> PageData<T> emptyPage(Integer pageSize, Integer pageNum){
        Pagination pagination = new Pagination(pageSize, pageNum, 0L, 0);
        return new PageData<T>(pagination, Collections.EMPTY_LIST);
    }
